...
export default {
  ...
  watch: {
    ...
    //     deep: true,
    //     handler() {
    //       this.order.discount_amount = this.valueTotal * (this.order.discount_percentage / 100);
    //     }
    // },
    // 'order.discount_amount': {
    //     deep: true,
    //     handler() {
    //       this.order.discount_percentage = (this.order.discount_amount * 100) / this.valueTotal;
    //     }
    // }
  },
  data: () => ({
  ...
  methods: {
    ...
    async _store(){
      this.$refs.fireDialog.loading({ title: 'Salvando...' });
